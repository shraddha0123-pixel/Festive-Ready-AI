...
type AgeGroup = "adult" | "teen" | "kid";
type GenderFit = "female" | "male" | "unisex";
...
export function FamilyParty() {
  ...
  const remainingSlots = 4 - partyMembers.length;

  const addMember = () => {
    if (!newName.trim() || partyMembers.length >= 4) return;

    const member: PartyMember = {
      id: `member-${Date.now()}`,
      name: newName.trim(),
      ageGroup: newAgeGroup,
      genderFit: newGenderFit,
    };

    setPartyMembers((current) => [...current, member]);
